...
function map_sync.gps:print_room_gps(room_id)
    if not room_id then
        error("wrong input in map_sync.gps:print_room_gps")
        return
    end

    local room_data_gps = getRoomUserData(room_id, map_sync.gps.user_data_gps_key)
    ...
    for k, gps_room_element in pairs(gps_room_elements) do
        local room_gps_id = map_sync.gps:generate_room_gps_id(room_id, k)
...
function map_sync.gps:add_gps_to_room(room_id, gps_str, line_delta, area_name, within_room_ids)
    if not room_id or not gps_str then
        error("wrong input in map_sync.gps:add_gps_to_room")
        return
    end

    local gps_string_lines = string.split(gps_str, map_sync.gps.gps_str_separator)
    ...
    setRoomUserData(room_id, map_sync.gps.user_data_gps_key, yajl_gps_room_elements)
...
function map_sync.gps:remove_gps(room_gps_id)
    local room_gps_id_array = string.split(room_gps_id, "_")
    if table.size(room_gps_id_array) ~= 2 then
    ...
    setRoomUserData(room_id, map_sync.gps.user_data_gps_key, yajl_filtered_gps_room_elements)
    cecho("\n<orange> Map Sync: ok, gps usuniety. Pamietaj o zapisie mapy przed wrzuceniem na serwer.\n")
end

function map_sync.gps:init_triggers()
    local gps_count = 0

    for id, name in pairs(getRooms()) do
        local room_data_gps = getRoomUserData(id, map_sync.gps.user_data_gps_key)
        if room_data_gps and room_data_gps ~= "" then
            local gps_data = yajl.to_value(room_data_gps)
            for k, v in pairs(gps_data) do
                map_sync.gps:init_trigger(id, k, v.gps_string_lines, v.line_delta, v.area_name, v.within_room_ids)
                gps_count = gps_count + 1
            end
        end
    end

    cecho("\n <orange> Map Sync: zbudowalem " .. tostring(gps_count) .. " elementow gps\n")
end

function map_sync.gps:init_trigger(room_id, room_incremental_gps_number, gps_string_lines, line_delta, area_name, within_room_ids)
    ...
    local room_gps_id = map_sync.gps:generate_room_gps_id(room_id, room_incremental_gps_number)
    ...
    if within_room_ids and table.size(within_room_ids) > 0 then
        map_sync.gps.room_id_to_within_room_ids[room_gps_id] = {}
        for _, room_id in pairs(within_room_ids) do
            map_sync.gps.room_id_to_within_room_ids[room_gps_id][tonumber(room_id)] = true
        end
    end

    map_sync.gps.room_id_to_gps_lines[room_gps_id] = gps_string_lines
    map_sync.gps.room_id_to_line_delta[room_gps_id] = calculated_line_delta
    ...
    map_sync.gps.trigger_ids[room_gps_id] = trigger_id
end
...
function map_sync_gps_first_line_match(room_id, room_gps_id, line_delta, area_name)
    if area_name and area_name ~= "" and area_name ~= amap.curr.area then
        ...
        return
    elseif map_sync.gps.room_id_to_within_room_ids[room_gps_id] ~= nil and map_sync.gps.room_id_to_within_room_ids[room_gps_id][tonumber(amap.curr.id)] == nil then
        -- not in the within room ids, ending
        return
    elseif table.size(map_sync.gps.room_id_to_gps_lines[room_gps_id]) == 1 then
        -- the gps consists of a single line
        amap:set_position(room_id, true)
        cecho("\n<orange> Map Sync: gps " .. room_gps_id)
    else
        map_sync.gps.current_room_id_gps_index[room_gps_id] = 2
        local temp_line_trigger_id = tempLineTrigger(1, line_delta, [[ map_sync_gps_subsequent_line_check_match("]] .. room_id .. [[", "]] .. room_gps_id .. [[")]])
        map_sync.gps.current_room_id_line_trigger_id[room_gps_id] = temp_line_trigger_id
    end
end

function map_sync_gps_subsequent_line_check_match(room_id, room_gps_id)
    local current_line_id = map_sync.gps.current_room_id_gps_index[room_gps_id]
    local gps_lines_count = table.size(map_sync.gps.room_id_to_gps_lines[room_gps_id])
    local line_matched_trigger = trim5(line) == map_sync.gps.room_id_to_gps_lines[room_gps_id][current_line_id]
    ...
    elseif line_matched_trigger then
        ...
        map_sync.gps.current_room_id_gps_index[room_gps_id] = map_sync.gps.current_room_id_gps_index[room_gps_id] + 1
    else
        -- didn't match, just kill the line trigger
        killTrigger(map_sync.gps.current_room_id_line_trigger_id[room_gps_id])
    end
end

function map_sync.gps:generate_room_gps_id(room_id, room_incremental_gps_number)
